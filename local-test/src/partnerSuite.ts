 ...
 * Used by local-test / playground so we can see PASS/FAIL live.
 */
...
export async function runPartnerSuite(
  client: ArcusXClient,
  opts: { gatewayLabel: string; apiKey: string },
): Promise<SuiteReport> {
  ...
  checks.push(
    await timed('escrow_list', 'partnerEscrow.list', async () => {
      const res = await client.partnerEscrow.list();
      const rows = Array.isArray((res as { escrows?: unknown[] }).escrows)
        ? (res as { escrows: unknown[] }).escrows
        : Array.isArray(res)
          ? res
          : [];
      return { pass: true, detail: `count=${rows.length}` };
  ...
  checks.push(
    await timed('deals_create', 'partnerDeals.create + getByToken', async () => {
      const payee =
        (typeof import.meta !== 'undefined' &&
          (import.meta as ImportMeta & { env?: Record<string, string> }).env
            ?.VITE_TEST_WORKER_WALLET) ||
        'GBXBJSGUDCUXED5FTRO63XIVYYWY4QVEIK6R2UGZ4SFGCJGUJA6HWE5H';
      const created = await client.partnerDeals.create({
        amountUsdc: 1,
        payeeWallet: payee,
        title: 'suite partner deal',
        externalId: `suite-deal-${Date.now()}`,
      });
      const token = String(
        (created as { deal_token?: string; deal?: { deal_token?: string } }).deal_token ??
          (created as { deal?: { deal_token?: string } }).deal?.deal_token ??
          '',
      );
      if (!token) return { pass: false, detail: 'missing deal_token' };
      const preview = await client.partnerDeals.getByToken(token);
      const ok = Boolean(
        (preview as { deal?: { deal_token?: string } }).deal?.deal_token ||
          (preview as { deal_token?: string }).deal_token,
      );
      return {
        pass: ok,
        detail: ok ? `token=${token.slice(0, 10)}…` : JSON.stringify(preview).slice(0, 120),
      };
    }),
  );
